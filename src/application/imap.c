...
static void imap_dump_v3(const struct ob_protocol* buffer)
{
...
static void imap_dump_v2(const struct ob_protocol* buffer)
{
    const unsigned char* hdr = buffer->hdr;
    if (buffer->length == 0)
    {
...
void imap_dump(struct ob_protocol* buffer)
{
